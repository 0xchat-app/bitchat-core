...
class MessageRetentionService {
    ...
    func saveMessage(_ message: BitchatMessage, forChannel channel: String?) {
        // Only save messages for favorite channels (simplified for now)
        ...
        let storedMessage = StoredMessage(
            ...
            senderPeerID: nil, // Simplified
            content: message.content,
            timestamp: message.timestamp,
            channelTag: channel,
            isPrivate: !message.isBroadcast,
            recipientPeerID: message.recipientId
        )
                ...
                if let encryptedData = try? Data(contentsOf: fileURL),
                   ...
                   let storedMessage = try? JSONDecoder().decode(StoredMessage.self, from: decryptedData) {
                    ...
                    let message = BitchatMessage(
                        id: storedMessage.id,
                        senderId: storedMessage.sender,
                        recipientId: storedMessage.recipientPeerID,
                        content: storedMessage.content,
                        type: .message,
                        timestamp: storedMessage.timestamp,
                        isBroadcast: !storedMessage.isPrivate
                    )
