...
class BluetoothMeshService: NSObject {
    ...
    internal var peerNicknames: [String: String] = [:]
    private let peerNicknamesLock = NSLock()
    internal var activePeers: Set<String> = []  // Track all active peers
    private let activePeersLock = NSLock()  // Thread safety for activePeers
    internal var peerRSSI: [String: NSNumber] = [:] // Track RSSI values for peers
    ...
    internal var peerLastSeenTimestamps: [String: Date] = [:]
    ...
    func sendBroadcastAnnounce() {
        // Simplified announce without nickname dependency
        let announcePacket = BitchatPacket(
            type: MessageType.announce.rawValue,
            ttl: 3,  // Increase TTL so announce reaches all peers
            senderID: myPeerID,
            payload: Data(myPeerID.utf8)
        )
...
extension BluetoothMeshService: CBPeripheralManagerDelegate {
    ...
    func sendMessage(_ packet: BitchatPacket, to recipientId: String) {
        // Create message packet
        let messagePacket = createMessagePacket(packet, recipientId: recipientId)
        
        // Send to connected peers
        for (peerId, peripheral) in connectedPeripherals {
            if peerId == recipientId {
                sendPacket(messagePacket, to: peripheral)
                break
            }
        }
        
        // Also broadcast for store-and-forward
        broadcastMessage(messagePacket)
    }
    
    /// Broadcast message to all peers
    func broadcastMessage(_ packet: BitchatPacket) {
        // Create broadcast packet
        let broadcastPacket = createBroadcastPacket(packet)
        
        // Send to all connected peers
        for peripheral in connectedPeripherals.values {
            sendPacket(broadcastPacket, to: peripheral)
        }
        
        // Store for future peers
        storeMessageForForwarding(broadcastPacket)
    }
    
    private func createMessagePacket(_ originalPacket: BitchatPacket, recipientId: String) -> BitchatPacket {
        // Create a new packet for specific recipient
    ...
    private func createBroadcastPacket(_ originalPacket: BitchatPacket) -> BitchatPacket {
        // Create a broadcast packet
        return BitchatPacket(
            type: originalPacket.type,
            senderID: myPeerID,
            recipientID: nil, // nil for broadcast
            timestamp: originalPacket.timestamp,
            payload: originalPacket.payload,
            signature: nil,
            ttl: originalPacket.ttl
    ...
    private func sendPacket(_ packet: BitchatPacket, to peripheral: CBPeripheral) {
    ...
    private func storeMessageForForwarding(_ packet: BitchatPacket) {
        // Store message for future peers
        let storedMessage = StoredMessage(
            packet: packet,
            timestamp: Date(),
        ...
        messageCache.append(storedMessage)
        
        // Limit cache size
        if messageCache.count > maxCachedMessages {
            messageCache.removeFirst()
        }
    }
    
    private func getCurrentPeerList() -> [PeerInfo] {
        return activePeers.map { peerId in
            PeerInfo(
                id: peerId,
                nickname: peerNicknames[peerId] ?? peerId,
                rssi: peerRSSI[peerId]?.intValue ?? 0,
                lastSeen: peerLastSeenTimestamps[peerId] ?? Date(),
                isConnected: connectedPeripherals[peerId] != nil
            )
        }
    }
}
