...
public struct BinaryProtocol {
    ...
    static func encode(_ packet: BitchatPacket) -> Data? {
        ...
        // Temporarily disable compression until CompressionUtil is available
    ...
    static func decode(_ data: Data) -> BitchatPacket? {
        ...
        if isCompressed {
            ...
            // Decompress
            // Temporarily disable decompression until CompressionUtil is available
            // guard let decompressedPayload = CompressionUtil.decompress(compressedPayload, originalSize: originalSize) else {
            //     return nil
            // }
            // payload = decompressedPayload
            payload = compressedPayload // Use compressed payload as-is for now
        } else {
...
extension BitchatMessage {
    func toBinaryPayload() -> Data? {
        ...
        if !originalSender.isEmpty { flags |= 0x04 }
        if !recipientNickname.isEmpty { flags |= 0x08 }
        if !senderPeerID.isEmpty { flags |= 0x10 }
        if !mentions.isEmpty { flags |= 0x20 }
        if !channel.isEmpty { flags |= 0x40 }
        ...
        // Optional fields
        if !originalSender.isEmpty, let origData = originalSender.data(using: .utf8) {
            data.append(UInt8(min(origData.count, 255)))
            data.append(origData.prefix(255))
        }
        
        if !recipientNickname.isEmpty, let recipData = recipientNickname.data(using: .utf8) {
            data.append(UInt8(min(recipData.count, 255)))
            data.append(recipData.prefix(255))
        }
        
        if !senderPeerID.isEmpty, let peerData = senderPeerID.data(using: .utf8) {
        ...
        // Channel hashtag
        if !channel.isEmpty, let channelData = channel.data(using: .utf8) {
